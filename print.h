...
struct _is_default_printable<
    T,
    std::void_t<std::pair<
        typename _printer<T>::is_default_print,
...
struct _enable_if_tuple<
    T, U, std::void_t<decltype(std::tuple_size<T>::value)>> {
    using type = U;
};
...
struct _enable_if_glmvec<
    ...
    std::enable_if_t<
        ...
        std::is_same_v<
            decltype(std::declval<typename T::bool_type>().x), bool>>> {
    using type = U;
};
    ...
    typename _enable_if_glmvec<
        ...
        typename _enable_if_glmvec<
            ...
            std::enable_if_t<
                ...
                std::is_same_v<
                    decltype(T()[0][0]), typename T::value_type &>>>::type>::
        type> {
    using type = U;
};
...
struct _enable_if_iterable<
    T, U,
    std::void_t<typename std::iterator_traits<decltype(
        std::begin(std::declval<T const &>()))>::value_type>> {
    using type = U;
};
...
struct _enable_if_has_print<
    T, U,
    std::void_t<decltype(
        std::declval<T const &>().do_print(std::declval<std::ostream &>()))>> {
    using type = U;
};
...
struct _enable_if_string<
    std::basic_string<T, Alloc, Traits>, U, typename _enable_if_char<T>::type> {
    using type = U;
};

template <class T, class Traits, class U>
struct _enable_if_string<
    std::basic_string_view<T, Traits>, U, typename _enable_if_char<T>::type> {
    using type = U;
};
...
struct _enable_if_c_str<
    ...
    std::enable_if_t<
        ...
        _is_char<std::remove_const_t<std::remove_pointer_t<std::decay_t<T>>>>::
            value>> {
    using type = U;
};
...
struct _printer<
    ...
    static void _unrolled_print(
        std::ostream &os, T const &t, std::index_sequence<Is...>) {
        os << "{";
        ((_printer<_rmcvref_t<std::tuple_element_t<Is, T>>>::print(
              os, std::get<Is>(t)),
          os << ", "),
         ...);
    ...
    static void print(std::ostream &os, T const &t) {
        _unrolled_print(
            ...
            std::make_index_sequence<
                std::max(static_cast<std::size_t>(1), std::tuple_size_v<T>) -
                1>{});
    }
};
...
class print_adaptor {
   ...
   public:
    ...
    friend std::ostream &operator<<(
        std::ostream &os, print_adaptor const &&self) {
        auto oldflags = os.flags();
        os << "[object 0x" << std::hex
           << reinterpret_cast<std::uintptr_t>(
                  reinterpret_cast<void const volatile *>(
                      std::addressof(self.t)))
           << ']';
        os.flags(oldflags);
        return os;
    }
